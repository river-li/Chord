...
from typing import List, Optional, TypedDict, Sequence, Annotated, Literal, Dict, Any
...
class TestingAgent:
    ...
    def preprocess(self):
        if self.enable_tool_filter:
            model_runnable = self.llm.bind_tools(self.filtered_tools)
        else:
    ...
    def tool_filter(self, state: TestingAgentState) -> TestingAgentState:
        ...
        with open(self.log_id+"_tool_filter_log.jsonl", 'a') as f:
            data = {"query": query, "original_tools": [t.name for t in self.tools], "filtered_tools": [t.name for t in new_tools]}
            json.dump(data, f)
            f.write("\n")
    ...
    def tool_node(self, state: TestingAgentState) -> TestingAgentState:
        ...
        for tool_call in tool_msg.tool_calls:
            if self.tool_cache_cursor:
                ...
                self.tool_cache_cursor.execute("SELECT result FROM tool_cache WHERE tool_name = ? AND args_hash = ?", (tool_call["name"], args_hash))
                ...
                if row:
                    ...
                    results.append(ToolMessage(content=observation, tool_call_id=tool_call["id"], name=tool_call["name"]))
                    self.tool_cache_cursor.execute(
                        "INSERT INTO tool_cache (tool_name, args_hash, result) VALUES (?, ?, ?)",
                        (tool_call["name"], args_hash, json.dumps(observation))
                    )
                    self.tool_cache_conn.commit()
            else:
                tool = tool_pool[tool_call["name"]]
...
if __name__ == '__main__':
    ...
    from Chord.evaluation.util import create_langchain_tool, create_run_function

    func = create_run_function("input", "Done")
    MaliciousTool = create_langchain_tool("MaliciousTool", tavily_malicious_tool["name"],
                                          tavily_malicious_tool["description"], func)

    DEBUG_AIRGAP = False
    DEBUG_PI_DETECTOR = False
    DEBUG_SPOTLIGHT = False
    DEBUG_TOOL_FILTER = True
    agent = None
